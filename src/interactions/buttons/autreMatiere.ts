...
const autreMatiereButton = new DiscordButton(new ButtonBuilder().setCustomId('autre_matiere').setLabel('Autre').setStyle(ButtonStyle.Secondary), false, async (interaction) => {
    ...
    const updatedFields = currentEmbed.fields.map(field => {
        if (field.name === 'Matière') {
            return { name: field.name, value: `✅ **Autre**`, inline: field.inline };
        }
...
export default autreMatiereButton;
