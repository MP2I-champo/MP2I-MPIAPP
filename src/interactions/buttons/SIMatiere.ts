...
const SIMatiereButton = new DiscordButton(new ButtonBuilder().setCustomId('si_matiere').setLabel('SI').setStyle(ButtonStyle.Primary), false, async (interaction) => {
    ...
    const updatedFields = currentEmbed.fields.map(field => {
        if (field.name === 'Matière') {
            return { name: field.name, value: `✅ **SI**`, inline: field.inline };
        }
...
export default SIMatiereButton;
